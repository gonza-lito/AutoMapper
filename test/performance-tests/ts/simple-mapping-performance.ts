import { AutoMapperInstance as automapper, IMemberConfigurationOptions } from '../../../dist';
import { IPerformanceTestResult } from './mapping-framework';

	export class SimpleMappingPerformance {
		public execute(repetitions: number = 1* 1000 * 1000): Array<IPerformanceTestResult> {
			var result = new Array<IPerformanceTestResult>();
		...
		private executeSimpleMapping(executeRepeatFunc: (action: () => void) => void, repetitions: number): IPerformanceTestResult {
			...
			automapper
				.createMap(from, to)
				.forMember('prop1', (opts: IMemberConfigurationOptions) => opts.sourceObject[opts.sourcePropertyName])
				.forMember('prop2', (opts: IMemberConfigurationOptions) => opts.sourceObject[opts.sourcePropertyName])
			;
