...
const AsyncAutoMapper_1 = require("./AsyncAutoMapper");
const AutoMapperBase_1 = require("./AutoMapperBase");
const AutoMapperEnumerations_1 = require("./AutoMapperEnumerations");
const AutoMapperHelper_1 = require("./AutoMapperHelper");
const AutoMapperValidator_1 = require("./AutoMapperValidator");
const TypeConverter_1 = require("./TypeConverter");
class AutoMapper extends AutoMapperBase_1.AutoMapperBase {
    /**
    ...
    constructor() {
        ...
        if (AutoMapper._instance) {
            ...
            this._profiles = {};
            this._mappings = {};
            this._asyncMapper = new AsyncAutoMapper_1.AsyncAutoMapper();
        }
    }
    static getInstance() {
        return AutoMapper._instance;
     ...
     */
    map(sourceKeyOrType, destinationKeyOrType, sourceObject) {
        ...
        if (arguments.length === 2) {
            return (srcObj) => this.mapInternal(super.getMapping(this._mappings, sourceKeyOrType, destinationKeyOrType), srcObj);
        }
        if (arguments.length === 1) {
            return (dstKey, srcObj) => this.map(sourceKeyOrType, dstKey, srcObj);
        }
        return (srcKey, dstKey, srcObj) => this.map(srcKey, dstKey, srcObj);
    }
    /**
     ...
     */
    mapAsync(sourceKeyOrType, destinationKeyOrType, sourceObject, callback) {
     ...
     */
    assertConfigurationIsValid(strictMode = true) {
        AutoMapperValidator_1.AutoMapperValidator.assertConfigurationIsValid(this._mappings, strictMode);
    }
    createMapForAllMembers(mapping, fluentFunc, func) {
        mapping.forAllMemberMappings.push(func);
        return fluentFunc;
    }
    createMapIgnoreAllNonExisting(mapping, fluentFunc) {
        mapping.ignoreAllNonExisting = true;
        return fluentFunc;
    }
    createMapConvertToType(mapping, fluentFunc, typeClass) {
    ...
    createMapConvertUsing(mapping, tcClassOrFunc) {
        const configureSynchronousConverterFunction = (converterFunc) => {
            ...
            mapping.mapItemFunction = (m, srcObj, dstObj) => this.mapItemUsingTypeConverter(m, srcObj, dstObj);
        };
        try {
    ...
    createMapWithProfile(mapping, profileName) {
    ...
    createMapWithProfileMergeMappings(mapping, profileName) {
        const profileMappingKey = `${profileName}=>${mapping.sourceKey}${profileName}=>${mapping.destinationKey}`;
        const profileMapping = this._mappings[profileMappingKey];
        if (!profileMapping) {
            return;
        }
    ...
    mapInternal(mapping, sourceObject) {
    ...
    mapArray(mapping, sourceArray) {
        const destinationArray = super.handleArray(mapping, sourceArray, (sourceObject, destinationObject) => {
            mapping.mapItemFunction(mapping, sourceObject, destinationObject);
        });
        return destinationArray;
    }
    mapItem(mapping, sourceObject, destinationObject) {
        destinationObject = super.handleItem(mapping, sourceObject, destinationObject, (propertyName) => {
            this.mapProperty(mapping, sourceObject, destinationObject, propertyName);
        });
        return destinationObject;
    }
    mapItemUsingTypeConverter(mapping, sourceObject, destinationObject, arrayIndex) {
    ...
    mapProperty(mapping, sourceObject, destinationObject, sourceProperty) {
        super.handleProperty(mapping, sourceObject, sourceProperty, destinationObject, (destinationProperty, options) => this.transform(mapping, sourceObject, destinationProperty, destinationObject, options));
    }
    transform(mapping, sourceObject, destinationProperty, destinationObject, options) {
    ...
    processTransformation(property, transformation, options) {
        switch (transformation.transformationType) {
            ...
            case AutoMapperEnumerations_1.DestinationTransformationType.SourceMemberOptions: {
                const result = transformation.sourceMemberConfigurationOptionsFunc(options);
                if (typeof result !== 'undefined') {
                    options.intermediatePropertyValue = result;
                }
                else if (!options.sourceObject) {
    ...
    createMappingObjectForGivenKeys(srcKeyOrType, dstKeyOrType) {
        const mapping = {
            ...
            mapItemFunction: (m, srcObj, dstObj) => this.mapItem(m, srcObj, dstObj),
    ...
    createMapGetFluentApiFunctions(mapping) {
        // create a fluent interface / method chaining (e.g. automapper.createMap().forMember().forMember() ...)
        const fluentFunc = {
            forMember: (prop, valFunc) => this.createMapForMember({ mapping, propertyName: prop, transformation: valFunc, sourceMapping: false, fluentFunctions: fluentFunc }),
            forSourceMember: (prop, cfgFunc) => this.createMapForMember({ mapping, propertyName: prop, transformation: cfgFunc, sourceMapping: true, fluentFunctions: fluentFunc }),
            forAllMembers: (func) => this.createMapForAllMembers(mapping, fluentFunc, func),
            ignoreAllNonExisting: () => this.createMapIgnoreAllNonExisting(mapping, fluentFunc),
            convertToType: (type) => this.createMapConvertToType(mapping, fluentFunc, type),
            convertUsing: (tcClassOrFunc) => this.createMapConvertUsing(mapping, tcClassOrFunc),
            withProfile: (profile) => this.createMapWithProfile(mapping, profile),
        };
        return fluentFunc;
    }
    createMapForMember(parameters) {
        ...
        const metadata = AutoMapperHelper_1.AutoMapperHelper.getMappingMetadataFromTransformationFunction(propertyName, transformation, sourceMapping);
    ...
    validateForMemberParameters(metadata) {
    ...
    createSourceProperty(metadata, parent) {
    ...
    createDestinationProperty(metadata, parent) {
        const level = !parent ? 0 : parent.level + 1;
    ...
    mergeSourceProperty(property, existingProperties, sourceMapping) {
     ...
     */
    handleMapFromProperties(property, existingProperty) {
        if (property.destinationPropertyName === property.sourcePropertyName ||
    ...
    getDestinationProperty(destinationPropertyName, existingSource) {
    ...
    mergeDestinationProperty(destination, existingDestination, swapTransformations = false) {
    ...
    matchSourcePropertyByDestination(source, properties) {
        if (!properties) {
            return null;
        }
